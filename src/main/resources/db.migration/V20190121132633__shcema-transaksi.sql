...
alter table transaksi add constraint fk_transaksi_toko foreign key (toko_id)
references toko (id) on update cascade on delete restrict;

alter table transaksi add constraint fk_transaksi_pelanggan foreign key (pelanggan_id)
references pelanggan (id) on on update cascade on delete restrict;
